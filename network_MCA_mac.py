...
class Dynamic_Target_Cross_Guidance(nn.Module):
    ...
    def forward(self, z, d, x):
        ...
        B, C, H, W = z.shape
        N = H*W
        Q = self.conv_q(z).view(B, C, N).permute(0, 2, 1)
...
class MCATracker:
    ...
    def init(self, frame_1, bbox_1):
        ...
        '''
        self.prev_frame = z_tensor
        self.prev_memory = torch.zeros_like(z_tensor)
        ...
        init_temp_256 =  self.neck(self.template_feat)   # [1, 256, 16, 16]
        print(f"압축 이후 : {init_temp_256.shape}")
        print(f"압축 이후 size : {init_temp_256.size}") 
        breakpoint()
        
        return init_temp_256
    ...
    def track(self, frame_t, bbox):
        ...
        search_region = self.magno_motion.forward(x_curr=c_t_f, x_prev = self.prev_frame, memory_prev = self.prev_memory)
        print(f"\n\nsearch region's shape : {search_region.shape}")
        breakpoint()
